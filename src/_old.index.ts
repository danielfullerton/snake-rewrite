...
interface SnakeBodypart extends Coordinatable {
  dir: Direction;
  index?: number;
}

type Snake = SnakeBodypart[];

const canvas = document.getElementById('canvas') as HTMLCanvasElement;
const ctx = canvas.getContext('2d');
let snake: Snake, candy: Partial<Coordinatable>, interval: number;
let score = 1;
let turnLock = false;
...
function makeCandy() {
  candy.x = getRandomPosition(0, 580);
  candy.y = getRandomPosition(0, 580);
}
...
document.body.addEventListener('keydown', function (e) {
  //DOWN
  if (e.key == 'ArrowDown' && dir !== 'U' && turnLock === false && dir !== 'D') {
    dir = 'D';
    turnLock = true;
  }
  //UP
  if (e.key == 'ArrowUp' && dir !== 'D' && turnLock === false && dir !== 'U') {
    dir = 'U';
    turnLock = true;
  }
  //RIGHT
  if (e.key == 'ArrowRight' && dir !== 'L' && turnLock === false && dir !== 'R') {
    dir = 'R';
    turnLock = true;
  }
  //LEFT
  if (e.key == 'ArrowLeft' && dir !== 'R' && turnLock === false && dir !== 'L') {
    dir = 'L';
    turnLock = true;
  }
  //RESET
